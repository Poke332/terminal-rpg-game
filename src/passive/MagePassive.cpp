...
void ArcanePower::onUnlock(Character& owner) {
    owner.modifyStat(Stat::attack, std::make_unique<AddModifier>(5.0f));
...
void BurningInsight::onUnlock(Character& owner) {
    ...
    owner.modifyStat(Stat::crit_damage, std::make_unique<AddModifier>(0.10f));
...
void GrandMagus::onUnlock(Character& owner) {
    owner.modifyStat(Stat::attack, std::make_unique<AddModifier>(10.0f));
    owner.modifyStat(Stat::crit_damage, std::make_unique<AddModifier>(0.05f));
