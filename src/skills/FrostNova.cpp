...
FrostNova::FrostNova()
    : Skill("Frost Nova",
            ...
            "single_cast_enemy", 10.0f, 0.3f, 5) {}

void FrostNova::skillImplementation(Character& user, Character& target) {
    float skillDamage = getFinalDamage(user.getAtkValue());
    target.takeDamage(skillDamage);
    target.modifyStat("attack", std::make_unique<PercentModifier>(-10.0f, 3));
    target.modifyStat("crit_chance", std::make_unique<PercentModifier>(-50.0f, 3));
